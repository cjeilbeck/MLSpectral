...
from FUNCTIONS.A_functions import moving_average, indicesmav, problemtype, read_data, customlabels
...
lines = ['-','--','-.',':']
for i, leaf_type in enumerate(data_avg.index):
    ...
    if HAIRCUT:
        ...
        y_plot = y_counts
    style = lines[i]
    label = customlabels.get(leaf_type, leaf_type)
    plt.plot(x_plot, y_plot, label=f"{label}",linestyle=style)
